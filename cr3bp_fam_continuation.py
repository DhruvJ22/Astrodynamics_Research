...
class periodic_orbit_fam_continuation(periodic_orbit):
    ...
    def npc_po_fam(self, free_vars, constraints, sym_period_targ=1 / 2, int_tol=1e-12, conv_tol=1e-12, Nmax=50, 
        ...
        while count_fam_member < num_fam_members and iterflag is False:
            ...
            # Targeter converged => save data and update the setup to compute the next family member
                ...
                self.ic = copy.copy(results["states"][0, :])  # To not update save data as values are passed as object reference
    ...
    def save_targeted_po_char(self, results):
        ...
        self.targeted_po_char["ic"].append(copy.copy(results["states"][0, :]))
        self.targeted_po_char["tf"].append(copy.copy(results["t"][-1]))
        self.targeted_po_char["jc"].append(copy.copy(self.JC(results["states"][0,:])))
